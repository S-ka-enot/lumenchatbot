"""
ВАРИАНТ B: Использование merge для синхронизации состояния
Подход: Используем merge для гарантированного обновления пользователя
Преимущества: Более надежная синхронизация состояния объекта
"""
...
async def extend_subscription(
    ...
    try:
        ...
        # ВАРИАНТ B: Используем expire_all для принудительной перезагрузки
...
async def create_subscriber(self, payload: SubscriberCreate) -> SubscriberListItem:
    try:
        ...
        if payload.subscription_days or payload.plan_id:
            ...
            # ВАРИАНТ B: Принудительная перезагрузка всех объектов
            self.session.expire_all()
            await self.session.refresh(user, attribute_names=["subscriptions", "subscription_end", "is_premium"])
...
async def _create_subscription_for_user(
    ...
    *,
    ...
    # ВАРИАНТ B: Используем expire для принудительной перезагрузки связей
    ...
    # Явно помечаем пользователя как измененного
    self.session.add(user)
    await self.session.flush()
    # Дополнительная перезагрузка для синхронизации
